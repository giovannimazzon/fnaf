...
#define SETC 8      
#define BREAKP 14   
#define CHKMEM 7    // Nova instrucao: varre a memoria procurando opcodes invalidos/lixo
...
int kbhit(void);
void verifica_lixo_memoria(void);
...
const char* dedo_duro(int op) {
	switch(op) {
		...
		case CHKMEM:  return "CHKMEM";
		default:      return "OPCODE_INVALIDO_OU_LIXO";
	}
}

// Nova instrucao CHKMEM: varre TODA a memoria (nao apenas o IR atual)
// procurando palavras cujo campo de opcode nao corresponde a nenhuma
// instrucao valida. Ao contrario de pane(), NAO aborta a execucao:
// apenas reporta e deixa o programa continuar.
void verifica_lixo_memoria(void) {
	int i, op;
	int total_lixo = 0;

	printf("\n[CHKMEM] Iniciando varredura de memoria (%d palavras)...\n", TAMANHO_MEMORIA);

	for (i = 0; i < TAMANHO_MEMORIA; i++) {
		op = pega_pedaco(MEMORY[i], 15, 10); // mesmo campo de opcode usado no DECODE

		if (strcmp(dedo_duro(op), "OPCODE_INVALIDO_OU_LIXO") == 0) {
			printf("  Endereco 0x%04X (%5d): opcode %2d invalido | palavra: 0x%04X\n",
			       i, i, op, MEMORY[i]);
			total_lixo++;
		}
	}
	...
	if (total_lixo == 0)
		printf("[CHKMEM] Nenhum opcode invalido encontrado. Memoria integra.\n\n");
	else
		printf("[CHKMEM] Varredura concluida: %d posicao(oes) com possivel lixo.\n\n", total_lixo);
}
