...
#define SETC 8      
#define BREAKP 14   
#define DEBUG 7    // Nova instrucao: varre a memoria procurando opcodes invalidos/lixo
...
// Nova instrucao DEBUG: varre TODA a memoria (nao apenas o IR atual)
...
void verifica_lixo_memoria(void) {
	...
	printf("\n[DEBUG] Iniciando varredura de memoria (%d palavras)...\n", TAMANHO_MEMORIA);
	...
	if (total_lixo == 0)
		printf("[DEBUG] Nenhum opcode invalido encontrado. Memoria integra.\n\n");
	else
		printf("[DEBUG] Varredura concluida: %d posicao(oes) com possivel lixo.\n\n", total_lixo);
}
